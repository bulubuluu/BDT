...
if __name__ == '__main__':
    ...
    for i in range(len(pid)):
        Z_mumu = reconstruct_Z_from_leptons(pt[i], eta[i], phi[i], mass[i], pid[i], 13, -13)
        Z_ee   = reconstruct_Z_from_leptons(pt[i], eta[i], phi[i], mass[i], pid[i], 11, -11)
        
        if Z_mumu is None or Z_ee is None:
            continue
        
        Z_mumu_list.append(Z_mumu)
        Z_ee_list.append(Z_ee)


    Z_mumu_array = np.array(Z_mumu_list)
    Z_ee_array   = np.array(Z_ee_list)

    z1_pt     = Z_mumu_array[:,0]
    z1_eta    = Z_mumu_array[:,1]
    z1_phi    = Z_mumu_array[:,2]
    z1_mass   = Z_mumu_array[:,3]
    z1_pz     = Z_mumu_array[:,4]
    z1_energy = Z_mumu_array[:,5]
    z2_pt     = Z_ee_array[:,0]
    z2_eta    = Z_ee_array[:,1]
    z2_phi    = Z_ee_array[:,2]
    z2_mass   = Z_ee_array[:,3]
    z2_pz     = Z_ee_array[:,4]
    z2_energy = Z_ee_array[:,5]
